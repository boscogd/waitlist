...
/**
 * Carrusel circular de los 3 móviles del hero. Se arrastra a izquierda y
 * derecha (ratón o dedo), tiene flechas, y gira solo de forma suave.
 * Respeta prefers-reduced-motion (sin giro automático).
 */
export default function PhoneCarousel() {
  ...
  return (
    ...
    >
      <div
        className="relative h-[360px] sm:h-[440px] lg:h-[480px] cursor-grab active:cursor-grabbing"
      ...
      >
        {PHONES.map((p, i) => {
          ...
          const x = Math.sin(rad); // -1..1 (posición horizontal)
          const depth = Math.cos(rad); // 1 = delante, -1 = detrás
          const translateX = x * 110;
          const scale = 0.62 + ((depth + 1) / 2) * 0.38;
          const rotateY = -x * 28;
            ...
            >
              ...
              >
                <Image
                  ...
                  width={240}
                  height={480}
                  className="rounded-[1.7rem] w-40 sm:w-48 lg:w-56 h-auto pointer-events-none"
                  priority={i === 0}
                  draggable={false}
                />
              </div>
